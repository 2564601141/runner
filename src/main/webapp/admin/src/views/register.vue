  ...
  <div>
    <div class="container">
      <div class="login-form" style="backgroundColor:rgba(255, 255, 255, 0.53);borderRadius:10px">
        <h1 class="h1" style="color:rgba(101, 126, 253, 1);fontSize:25px;">校园跑腿系统小程序注册</h1>
		<el-form ref="rgsForm" class="rgs-form" :model="rgsForm" label-width="120px">
			...
			<!-- <div v-if="tableName=='paotuizhe'" class="input-group">
			   <div class="label">工号</div>
			   <div class="input-container">
			     <input v-model="ruleForm.gonghao" class="input" type="text" placeholder="工号">
			   </div>
			 </div> -->
			<el-form-item label="工号" class="input" v-if="tableName=='paotuizhe'">
			...
			<el-form-item label="密码" class="input" v-if="tableName=='paotuizhe'">
			  <el-input v-model="ruleForm.mima" autocomplete="off" placeholder="密码" type="text" />
			</el-form-item>
			<!-- <div v-if="tableName=='paotuizhe'" class="input-group">
			...
			<el-form-item label="姓名" class="input" v-if="tableName=='paotuizhe'">
			...
			<el-form-item label="手机" class="input" v-if="tableName=='paotuizhe'">
			  <el-input v-model="ruleForm.shouji" autocomplete="off" placeholder="手机" type="text" />
			</el-form-item>
			<!-- <div v-if="tableName=='paotuizhe'" class="input-group">
			   <div class="label">邮箱</div>
			...
			<el-form-item label="邮箱" class="input" v-if="tableName=='paotuizhe'">
			  <el-input v-model="ruleForm.youxiang" autocomplete="off" placeholder="邮箱" type="text" />
			</el-form-item>
			<!-- <div v-if="tableName=='paotuizhe'" class="input-group">
			   <div class="label">身份证</div>
			...
			<el-form-item label="身份证" class="input" v-if="tableName=='paotuizhe'">
			  <el-input v-model="ruleForm.shenfenzheng" autocomplete="off" placeholder="身份证" type="text" />
			</el-form-item>
...
export default {
  ...
  methods: {
    ...
    login() {
      ...
      if((!this.ruleForm.gonghao) && `paotuizhe` == this.tableName){
        this.$message.error(`工号不能为空`);
        return
      }
      if((!this.ruleForm.mima) && `paotuizhe` == this.tableName){
        this.$message.error(`密码不能为空`);
        return
      }
      if((!this.ruleForm.shouji) && `paotuizhe` == this.tableName){
        this.$message.error(`手机不能为空`);
        return
      }
      if(`paotuizhe` == this.tableName && this.ruleForm.shouji&&(!this.$validate.isMobile(this.ruleForm.shouji))){
        this.$message.error(`手机应输入手机格式`);
        return
      }
      if(`paotuizhe` == this.tableName && this.ruleForm.youxiang&&(!this.$validate.isEmail(this.ruleForm.youxiang))){
        this.$message.error(`邮箱应输入邮件格式`);
        return
      }
      if((!this.ruleForm.shenfenzheng) && `paotuizhe` == this.tableName){
        this.$message.error(`身份证不能为空`);
        return
      }
      if(`paotuizhe` == this.tableName && this.ruleForm.shenfenzheng&&(!this.$validate.checkIdCard(this.ruleForm.shenfenzheng))){
        this.$message.error(`身份证应输入身份证格式`);
        return
      }
      this.$http({
